...
#         imgx = cv2.imread(valid_image_file_list[ino])
#         # Get the dimensions of the image
#         image_height, image_width = imgx.shape[:2]
...
#         # Calculate the height of the canvas (image height + text height + some padding)
#         padding = 20
#         canvas_height = image_height + text_size[1] + padding
...
#         cv2.putText(canvas, title, (text_x, text_y), font, font_scale, (0, 0, 0), font_thickness, lineType=cv2.LINE_AA)

#         # Place the image below the text
#         image_y_start = text_y + padding // 2
#         image_y_end = image_y_start + image_height
#         canvas[image_y_start:image_y_end, 0:image_width] = imgx
#         cv2.imwrite(f"/home/frinks2/subhra/PaddleOCR/inference/results/{valid_image_file_list[ino].split('/')[-1][:-4]}_res.png",canvas)
...
--image_dir=""  
--rec_model_dir=./inference/rec_ppocrv3_skh_jun27 
--use_gpu True 
...
class PaddleOCRx():
    ...
    def __init__(self,model_weights,rec_image_shape="3, 48, 320",rec_char_dict_path="/home/frinksserver/Deepak/yolo_fasterrcnn_trocr_modular/model_weights/paddleocr/en_dict.txt" ) -> None:
        ...
        if self.args.warmup:
            img = np.random.uniform(0, 255, [48, 320, 3]).astype(np.uint8)
            for i in range(2):
                res = self.modelpaddlex([img] * int(self.args.rec_batch_num))

        print(f"[INFO] {datetime.datetime.now()}: paddleOCR loaded!!!\n")
    ...
    def ocrx(self, image):
        ...
        # print(f"\n\n ---------------------------------- {img_list} --------------------------")
        try:
            rec_resx, _ = self.modelpaddlex(img_list)
